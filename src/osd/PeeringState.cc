  ...
  bool require_target_usage)
{
  ...
  for (const auto& acting : actingset) {
    ...
    if (!usage || usage->total_bytes == 0) {
      psdout(20) << "missing OSD space usage for relieved " << acting
		 << " target " << target << dendl;
      return std::nullopt;
    }
  ...
  if (target_usage && target_usage->total_bytes > 0) {
    ...
    space_info.target_usage_before = target_usage->usage_ratio();
...
unsigned PeeringState::get_space_aware_local_backfill_priority()
{
  ...
  unsigned priority = base_priority;
  for (const auto& target : backfill_targets) {
    priority = std::max(
      priority,
      apply_backfill_space_priority(
	base_priority,
	get_backfill_reservation_space_info(target, true)));
  }
  return priority;
}
...
PeeringState::WaitRemoteBackfillReserved::react(const RemoteBackfillReserved &evt)
{
  ...
  if (backfill_osd_it !=
    ...
    auto space_info =
      ps->get_backfill_reservation_space_info(*backfill_osd_it);
    pl->send_cluster_message(
      backfill_osd_it->osd,
      TOPNSPC::make_message<MBackfillReserve>(

...
public:
  ...
  //   ENCODE_START(1, 1, bl);
  //   ::ceph::encode(reinterpret_cast<const char *>(&value), bl);
  //   ENCODE_FINISH(bl);
  // }
...
// inline void encode(const laddr_t& a, ::ceph::bufferlist& bl) {
//   ::ceph::encode(a.value, bl);  // encode the underlying uint64_t
// }
// inline void decode(laddr_t& a, ::ceph::bufferlist::const_iterator& p) {
...
struct SnapSet {
  snapid_t seq;
  std::vector<snapid_t> clones;   // ascending 所有clone的snapid
  std::map<snapid_t, interval_set<uint64_t> > clone_overlap;  // overlap w/ next newest
  std::map<snapid_t, uint64_t> clone_size;
  std::map<snapid_t, std::vector<snapid_t>> clone_snaps; // descending 创建当前clone时。对当前clone可见的所有snapid
  ...
  bool operator!=(const SnapSet& other) const {
    return !(*this == other);
  }
  ...
  bool operator==(const SnapSet& other) const {
    return seq == other.seq &&
          clones == other.clones &&
          clone_overlap == other.clone_overlap &&
          clone_size == other.clone_size &&
          clone_snaps == other.clone_snaps;
  }

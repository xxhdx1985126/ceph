...
template <typename InterruptCond, typename FutureType, typename T =std::conditional_t<
...
class parallel_for_each_state final : private seastar::continuation_base<> {
  using maybe_embedded_future_type = std::conditional_t<
    is_interruptible_future<FutureType>::value,
    typename FutureType::core_type,
    FutureType>;
  using future_t = interruptible_future_detail<
    InterruptCond,
    typename parallel_for_each_ret<maybe_embedded_future_type>::type>;
...
public:
  ...
  static auto now() {
    if constexpr (seastar::is_future<maybe_embedded_future_type>::value) {
      return interruptor<InterruptCond>::now();
    } else {
      return interruptible_errorator<
	InterruptCond,
	typename maybe_embedded_future_type::errorator_type>::now();
    }
  }
};
...
private:
  ...
  template <typename IC, typename FT, typename>

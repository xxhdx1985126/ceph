  ...
  : path{path}
{
  ...
  tp = std::make_unique<crimson::os::ThreadPool>(threadpool_size, 128, cpu_id);
}
                        ...
                        uint64_t limit) const
{
  ...
  return seastar::do_with(std::vector<ghobject_t>(), ghobject_t(),
    ...
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size),
      [=, &objects, &next] {
...
{
  logger().debug("{}", __func__);
  return tp->submit(::rand() % threadpool_size, [this, cid] {
...
seastar::future<std::vector<coll_t>> AlienStore::list_collections()
{
  ...
  return seastar::do_with(std::vector<coll_t>{}, [=] (auto &ls) {
    return tp->submit(::rand() % threadpool_size, [this, &ls] {
                 ...
                 uint32_t op_flags)
{
  ...
  return seastar::do_with(ceph::bufferlist{}, [=] (auto &bl) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [=, &bl] {
      auto c = static_cast<AlienCollection*>(ch.get());
                     ...
                     std::string_view name) const
{
  ...
  return seastar::do_with(ceph::bufferptr{}, [=] (auto &value) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [=, &value] {
      auto c =static_cast<AlienCollection*>(ch.get());
                      ...
                      const ghobject_t& oid)
{
  ...
  return seastar::do_with(attrs_t{}, [=] (auto &aset) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [=, &aset] {
      auto c = static_cast<AlienCollection*>(ch.get());
  ...
  -> read_errorator::future<omap_values_t>
{
  ...
  return seastar::do_with(omap_values_t{}, [=] (auto &values) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [=, &values] {
      auto c = static_cast<AlienCollection*>(ch.get());
...
{
  logger().debug("{}", __func__);
  return tp->submit(::rand() % threadpool_size, [=] {
...
{
  logger().debug("{}", __func__);
  return tp->submit(::rand() % threadpool_size, [this, key] {
    std::string value;
...
seastar::future<store_statfs_t> AlienStore::stat() const
{
  ...
  return seastar::do_with(store_statfs_t{}, [this] (store_statfs_t &st) {
    return tp->submit(::rand() % threadpool_size, [this, &st] {
  ...
  const ghobject_t& oid)
{
  return seastar::do_with((struct stat){}, [this, ch, oid](auto& st) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [this, ch, oid, &st] {
      auto c = static_cast<AlienCollection*>(ch.get());
  ...
  uint64_t len)
{
  return seastar::do_with(std::map<uint64_t, uint64_t>(), [=](auto& destmap) {
    return tp->submit(ch->get_cid().hash_to_shard(threadpool_size), [=, &destmap] {
      auto c = static_cast<AlienCollection*>(ch.get());
...
{
  return store->tp->submit(ch->get_cid().hash_to_shard(store->threadpool_size), [this] {
...
{
  return store->tp->submit(ch->get_cid().hash_to_shard(store->threadpool_size), [this, after] {
...
seastar::future<> AlienStore::AlienOmapIterator::lower_bound(
  const std::string& to)
{
  return store->tp->submit(ch->get_cid().hash_to_shard(store->threadpool_size), [this, to] {

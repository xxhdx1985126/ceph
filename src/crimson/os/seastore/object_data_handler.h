...
private:
    ...
    op_type_t op_type) const
  {
    if (edge_mapping.is_pending()) {
      return edge_handle_policy_t::MERGE_PENDING;
    }
    if (should_delta_based_overwrite(edge_mapping, start, len)
	&& op_type != op_type_t::TRIM) {
      return edge_handle_policy_t::DELTA_BASED_PUNCH;
    }
    return edge_handle_policy_t::REMAP;
  }

  bool should_delta_based_overwrite(
  ...
  base_iertr::future<LBAMapping> do_delta_based_edge_punch(
    context_t ctx,
    overwrite_range_t &overwrite_range,
    data_t& data,

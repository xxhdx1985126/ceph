  ...
  CachedExtentRef extent)
{
  ...
  return seastar::do_with(
    ...
    [&t, this, FNAME](std::vector<CachedExtentRef> &promoted_extents,
		      ...
		      extent_len_t &offset) {
      return trans_intr::do_for_each(promoted_extents, [&t, &retired_lextent, &offset, this, FNAME](CachedExtentRef &extent) {
	auto lextent = extent->cast<LogicalCachedExtent>();
	...
	return lba_manager->alloc_extent(
	  ...
	  *cold_extent,
	  ...
	  {.determinsitic=true}
	).si_then([&t, &retired_lextent, &offset, lextent, cold_extent, this, FNAME](auto mapping) {
	  ...
	  if (offset == 0) {
	    ...
	    fut = lba_manager->update_mapping(
	      ...
	      lextent.get(),
	      /*has_shadow=*/true).discard_result();
	  } else {

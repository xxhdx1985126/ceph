...
public:
  ...
  void segment_used(segment_id_t segment) {
  ...
  void update_segment_avail_bytes(paddr_t offset) {
	...
	"SegmentCleaner::update_segment_avail_bytes: segment closed {}, not updating", offset);
      return;
    }
    auto new_avail_bytes = sm_info->segment_size - offset.as_seg_paddr().get_segment_off();
    if (iter->second < new_avail_bytes) {
      crimson::get_logger(ceph_subsys_seastore).info(
	"SegmentCleaner::update_segment_avail_bytes: avail_bytes increased? , {}, {}", iter->second, new_avail_bytes);
...
public:
  ...
  int64_t allocate(
    ...
    live_bytes_by_segment[segment].total_bytes += len;
  ...
  double calc_utilization(segment_id_t segment) const final {
    ...
    if (!seg_bytes.live_bytes && !seg_bytes.total_bytes)
      return 0;
    assert(seg_bytes.total_bytes);
...
class SpaceTrackerDetailed : public SpaceTrackerI {
  ...
  public:
    ...
    double calc_utilization() const {
      if (!used && !total_occupied)
	return 0;
      assert(total_occupied);
      return (double)used / (double)total_occupied;
    }
...
public:
  ...
  struct config_t {
    ...
    static config_t get_default() {
	  ...
	  1<<26,// reclaim 64MB per gc cycle
	  1<<26 // rewrite 64MB of journal entries per gc cycle
	};
    }
  };
...
private:
  ...
  struct {
    ...
    uint64_t accumulative_blocked_ios = 0;
...
public:
  ...
  void mount(device_id_t pdevice_id, std::vector<SegmentManager*>& sms) {
    ...
    metrics.clear();
    stats.avail_bytes = stats.total_bytes = segments.get_total_bytes();
    register_metrics();
  }
  ...
  void update_segment_avail_bytes(paddr_t offset) final {
    ...
    stats.avail_bytes = segments.get_available_bytes(
      offset.as_seg_paddr().get_segment_id().device_id());
  }
  ...
  void mark_space_used(
    ...
    stats.segment_util.buckets[
      old_usage > 0.5 ?
	std::lround(old_usage * 10 - 0.5) :
	std::lround(old_usage * 10)].count--;
    stats.segment_util.buckets[
      new_usage > 0.5 ?
...
public:
  seastar::future<> reserve_projected_usage(size_t projected_usage) {
	...
	return !should_block_on_gc();
      ...
      [this] {
	stats.accumulative_blocked_ios ++;
...
private:
  ...
  void mark_closed(segment_id_t segment) {
    ...
    stats.avail_bytes = segments.get_available_bytes(segment.device_id());
    stats.opened_segments = segments.get_opened_segments(segment.device_id());
    crimson::get_logger(ceph_subsys_seastore).info(

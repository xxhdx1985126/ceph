  ...
  std::vector<CachedExtentRef> &extents)
{
  return seastar::do_with(
	...
	t, ent.type, ent.paddr, ent.laddr, ent.len
      ).si_then([this, FNAME, &extents, &ent, &t](auto ext) {
	if (!ext) {
  ...
  return seastar::do_with(
	...
	return repeat_eagain(
	  ...
	  return ecb->with_transaction_intr(
		...
		for (auto it = backref_set.first; it != backref_set.second; it++) {
		  auto &backref = *it;
		...
		auto backref_set = 
		  backref_manager.get_cached_backrefs_in_range(
		    reclaim_state->start_pos, reclaim_state->end_pos);
		return backref_manager.merge_backrefs(t, backref_set);
		...
		return trans_intr::do_for_each(
	      ...
	      last_reclaimed_to.segment_seq = get_seg_info(
		reclaim_state->start_pos.as_seg_paddr().get_segment_id()).seq;
	      ceph_assert(last_reclaimed_to.segment_seq != NULL_SEG_SEQ);
	      last_reclaimed_to.offset = reclaim_state->end_pos;

...
SegmentCleaner::clean_space_ret SegmentCleaner::clean_space()
{
  ...
  return seastar::do_with(
    ...
    backref_pin_list_t(),
    [this, FNAME, pavail_ratio, start](auto& backref_extents, auto& pin_list) {
    return repeat_eagain([this, &backref_extents, &pin_list] {
      return extent_callback->with_transaction_intr(
        Transaction::src_t::READ,
        "retrieve_from_backref_tree",
        [this, &backref_extents, &pin_list](auto &t) {
        return backref_manager.get_mappings(
          t,
          reclaim_state->start_pos,
          reclaim_state->end_pos
        ).si_then([this, &t, &backref_extents, &pin_list](auto pin_list_) {
	  pin_list = std::move(pin_list_);
          if (!pin_list.empty()) {
          ...
          return backref_manager.retrieve_backref_extents_in_range(
            ...
            reclaim_state->start_pos,
            reclaim_state->end_pos
          ).si_then([&backref_extents](auto extents) mutable {
	    backref_extents = std::move(extents);
          });
        });
      });
    }).safe_then([this, FNAME, pavail_ratio, start, &backref_extents, &pin_list]() {
      return seastar::do_with(
        (size_t)0,
        (size_t)0,
        [this, FNAME, pavail_ratio, start,
	 &backref_extents, &pin_list](auto &reclaimed, auto &runs)
      {

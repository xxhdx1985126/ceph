...
struct FixedKVNode : CachedExtent {
  ...
  // only used in mutation pending fixed-kv-nodes
  ...
  void mutate_push_back(
    pending_children_set_t::iterator begin,
    pending_children_set_t::iterator end,
    int16_t pos_shift) {
    ...
    FixedKVNode &src)
  {
	...
	src.get_prior_instance()->cast<FixedKVNode>());
    } else {
  ...
  void set_parent_tracker() {
    ...
    if (pin.is_root()) {
      ...
      set_phy_tree_root_node<false, node_key_t>(root_block, this);
      return;
    }
...
struct FixedKVInternalNode
  ...
  using base_ref = FixedKVNode<NODE_KEY>::FixedKVNodeRef;
  using node_layout_t =
    common::FixedKVNodeLayout<
      CAPACITY,
      fixed_kv_node_meta_t<NODE_KEY>,

...
public:
  ...
  static mkfs_ret mkfs(op_context_t<node_key_t> c) {
    ...
    fixed_kv_node_meta_t<node_key_t> meta{0, max_key_t<node_key_t>::max, 1};
  ...
  iterator_fut end(op_context_t<node_key_t> c) const {
    return upper_bound(c, max_key_t<node_key_t>::max);
  }
  ...
  insert_ret insert(
    ...
    SUBDEBUGT(
      ...
      c.trans,
      laddr,
      iter.is_end() ? max_key_t<node_key_t>::max : iter.get_key());
    return seastar::do_with(
      iter,
    ...
    iterator &iter)
  {
    ...
    if (split_from == iter.get_depth()) {
      ...
      fixed_kv_node_meta_t<node_key_t> meta{
        0, max_key_t<node_key_t>::max, iter.get_depth() + 1};
      nroot->set_meta(meta);

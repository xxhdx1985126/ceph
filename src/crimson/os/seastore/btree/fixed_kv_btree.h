    ...
    typename internal_node_t::base_t &next_extent)
  {
    ...
    return lower_bound(
      ...
      if (depth == iter.get_depth()) {
        ...
        ceph_assert(next_extent.is_pending());
...
private:
  ...
  static get_internal_node_ret get_internal_node(
    ...
    node_key_t begin,
    node_key_t end,
    std::optional<node_position_t<internal_node_t>> parent_pos = std::nullopt)
  {
    ...
    auto init_internal = [c, depth, begin, end,
      ...
      if (parent_pos) {
        //TODO: this code block can be reused by get_leaf_node,
        //      extract it into dedicated a private method
        ...
        if (parent->is_pending_by_me(c.trans.get_trans_id())) {
          auto orig_node = tracker.get_prior_instance();
          ceph_assert(orig_node);
          auto orig_parent_node = orig_node->template cast<internal_node_t>();
          auto cur_iter = parent->iter_idx(parent_pos->pos);
          auto orig_iter = orig_parent_node->lower_bound(cur_iter.get_key());
          auto &orig_ptracker = orig_parent_node->get_child_tracker(
            orig_iter.get_offset());
          if (orig_ptracker.is_empty()) {
            orig_ptracker.update_child(&node);
            node.parent_tracker = &orig_ptracker;
          } else {
            ceph_assert(orig_ptracker.get_child_global_view() == &node);
            ceph_assert(node.parent_tracker == &orig_ptracker);
          }
          auto [it, inserted] = node.parent_tracker_trans_views.insert(tracker);
    ...
    auto init_leaf = [c, begin, end,
      ...
      if (parent_pos) {
        ...
        if (parent->is_pending_by_me(c.trans.get_trans_id())) {
          auto orig_node = tracker.get_prior_instance();
          ceph_assert(orig_node);
          auto orig_parent_node = orig_node->template cast<leaf_node_t>();
          auto cur_iter = parent->iter_idx(parent_pos->pos);
          auto orig_iter = orig_parent_node->lower_bound(cur_iter->get_key());
          auto &orig_ptracker = orig_parent_node->get_child_tracker(
            orig_iter.get_offset());
  ...
  static lookup_internal_level_ret lookup_internal_level(
    ...
    auto end = next_iter == parent->end()
      ...
      : next_iter->get_key();
    assert(c.trans.get_extent(
      node_iter->get_val().maybe_relative_to(parent->get_paddr()),
      nullptr) == Transaction::get_extent_ret::ABSENT);
  ...
  static lookup_internal_level_ret lookup_leaf(
    ...
    auto on_found =
      [visitor, &iter, &f](LeafNodeRef node) {
      iter.leaf.node = node;
      auto node_iter = f(*node);
    ...
    return get_leaf_node<false>(
      ...
      std::make_optional<node_position_t<internal_node_t>>(parent_entry)
    ).si_then([visitor, &iter, &f, on_found=std::move(on_found)]
      (LeafNodeRef node) {
      return on_found(node);
    });
  }
    ...
    iterator &iter)
  {
    ...
    auto split_level = [&](auto &parent_pos, auto &pos) {
      ...
      parent_node->insert(
        ...
        pivot,
        right->get_paddr());
      auto &l_tracker = parent_node->get_child_tracker(parent_pos.pos);
            ...
            return merge_fut.si_then([FNAME, this, c, &iter, &to_merge] {
              ...
              if (to_merge == iter.get_depth()) {
                if (pos.node->get_size() == 1) {
                  ...
                  ceph_assert(new_root->is_pending());
                  c.cache.retire_extent(c.trans, pos.node);
                  assert(pos.pos == 0);
  ...
  base_iertr::future<typename NodeType::Ref> get_node(
    ...
    node_key_t begin,
    node_key_t end,
    std::optional<node_position_t<internal_node_t>> parent_pos) {
    assert(depth == 1);
    return get_leaf_node<false>(
      c, addr, begin, end, std::move(parent_pos));
  }
    ...
    iterator &tree_iter)
  {
    ...
    assert(c.trans.get_extent(
      ...
      nullptr) == Transaction::get_extent_ret::ABSENT);
    auto next_parent_pos = parent_pos;
    if (donor_is_left)
      next_parent_pos.pos--;
    else
      next_parent_pos.pos++;
    return get_node<NodeType>(
      ...
      std::make_optional<node_position_t<internal_node_t>>(std::move(next_parent_pos))
    ).si_then([c, iter, donor_iter, donor_is_left, &parent_pos,
      &pos, do_merge=std::move(do_merge)](typename NodeType::Ref donor) {

    ...
    for (auto i : *node) {
      ...
      if (ret == Transaction::get_extent_ret::PRESENT) {
        if (child_node->is_mutation_pending()) {
          ...
          assert(cnode->parent_tracker);
          assert(cnode->parent_tracker->get_parent().get() == node.get());
        } else {
          ...
          if (node->is_pending()) {
            auto &n = node->get_stable_for_key(i->get_key());
            assert(cnode->parent_tracker->get_parent().get() == &n);
            auto pos = n.lower_bound_offset(i->get_key());
            assert(pos < n.get_node_size());
        ...
        if (node->is_pending()) {
          ...
          if (child) {
            ...
            assert(c->parent_tracker);
            assert(c->parent_tracker->get_parent().get() == &n);
          }
        } else {
          ...
          if (child) {
            auto c = (child_node_t*)child;
            assert(c->parent_tracker);
            assert(c->parent_tracker->get_parent().get() == node.get());
          }
        }
    ...
    auto init_internal = [c, depth, begin, end,
      ...
      if (parent_pos) {
        ...
        if (root_block->is_mutation_pending()) {
          ...
          set_phy_tree_root_node<false, self_type>(stable_root, &node);
        } else {

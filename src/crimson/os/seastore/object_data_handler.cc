...
std::ostream& operator<<(std::ostream &out, const overwrite_range_t &overwrite_range) {
  return out << "overwrite_range_t{"
    << "unaligned_len=" << overwrite_range.unaligned_len
    << ", unaligned_begin=" << overwrite_range.unaligned_begin
    << ", aligned_begin=" << overwrite_range.aligned_begin
    << ", unaligned_end=" << overwrite_range.unaligned_end
    << ", aligned_end=" << overwrite_range.aligned_end
    << ", aligned_len=" << overwrite_range.aligned_len
    << "}";
}
...
ObjectDataHandler::do_delta_based_edge_punch(
  context_t ctx,
  overwrite_range_t &overwrite_range,
  data_t& data,
  LBAMapping edge_mapping,
  ...
  edge_t edge)
{
  ...
  return fut.si_then([ctx, edge_mapping, &overwrite_range, edge] {
    ...
    } else if (overwrite_range.aligned_end <
  ...
  op_type_t op_type)
{
  ...
  case edge_handle_policy_t::DELTA_BASED_PUNCH:
    return do_delta_based_edge_punch(
      ctx, overwrite_range, data, std::move(edge_mapping), edge);
  case edge_handle_policy_t::MERGE_PENDING:
...
ObjectDataHandler::handle_single_mapping_overwrite(
  ...
  LBAMapping mapping)
{
  if (should_delta_based_overwrite(
	mapping,
	overwrite_range.aligned_begin,

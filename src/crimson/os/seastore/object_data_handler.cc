  ...
  object_data_t &d_object_data)
{
  ...
  ceph_assert(ctx.onode.is_head());
  return prepare_shared_data_reservation(
    ctx, *ctx.d_onode, d_object_data, object_data.get_reserved_data_len()
  ).si_then([ctx, &object_data, &d_object_data](auto prepared_region) {
    ceph_assert(!prepared_region.private_region.is_null());
    ceph_assert(!prepared_region.shared_region.is_null());
    return seastar::do_with(
      state_t {
	LBAMapping{},
	LBAMapping{},
	...
	ctx.d_onode->get_shared_region_base()},

  ...
  extent_len_t size)
{
  ...
  return ctx.tm.reserve_region(
    ctx.t,
    onode.get_data_hint(),
    get_clone_reservation_length()
  ).si_then([ctx, this, &onode](auto pin) {
    ceph_assert(pin.get_length() == get_clone_reservation_length());
...
ObjectDataHandler::touch_ret ObjectDataHandler::touch(context_t ctx)
{
  ...
  return with_object_data(
    ...
    return prepare_data_reservation(
      ctx, ctx.onode, object_data, max_object_size
    ).discard_result();
  });
}
  ...
  extent_len_t len)
{
  ...
  return seastar::do_with(
	...
	return move_mappings_iertr::make_ready_future<
      ...
      if (mapping.is_indirect() || mapping.get_val().is_zero()) {
	DEBUGT("skipping mapping {}", ctx.t, mapping);
	return ctx.tm.next_mapping(ctx.t, std::move(mapping)
	).si_then([&mapping](auto m) {
	  mapping = std::move(m);
	  return seastar::stop_iteration::no;
	});
      }
  ...
  context_t ctx)
{
  ...
  return with_objects_data(
    ...
    return prepare_data_reservation(
      ctx, *ctx.d_onode, d_object_data,
      object_data.get_reserved_data_len()
    ).si_then([ctx, &object_data, &d_object_data, this](auto) {
      return seastar::do_with(
	state_t {
	  LBAMapping{},
	  LBAMapping{},

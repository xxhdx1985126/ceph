  ...
  extent_len_t dstoff)
{
  ...
  return with_objects_data(
    ...
    return ctx.tm.get_pins(ctx.t, src_start, length
	...
	return ctx.tm.get_pins(ctx.t, base, len
	  ...
	  return seastar::do_with(
	    ...
	    return ctx.tm.remove(
	      ctx.t,
	      object_data.get_reserved_data_base()
	    ).si_then([ctx, &object_data, &pins, base, this](auto) {
	      return clone_extents(ctx, object_data, pins, base, 0, 0);
	    }).si_then([ctx, &d_object_data] {
	      return ctx.tm.remove(
		ctx.t,
		d_object_data.get_reserved_data_base());
	    }).si_then([ctx, &d_object_data, base, &pins, this](auto) {
	      return clone_extents(ctx, d_object_data, pins, base, 0, 0);
	    }).si_then([&pins, ctx] {

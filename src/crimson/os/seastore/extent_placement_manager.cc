  ...
  segment_off_t length)
{
  ...
  case extent_types_t::ONODE_BLOCK_STAGED:
    return alloc_ool_extent<onode::SeastoreNodeExtent>(hint, length);
  case extent_types_t::OMAP_INNER:
    return alloc_ool_extent<omap_manager::OMapInnerNode>(hint, length);
  case extent_types_t::OMAP_LEAF:
    return alloc_ool_extent<omap_manager::OMapLeafNode>(hint, length);
  case extent_types_t::COLL_BLOCK:
    return alloc_ool_extent<collection_manager::CollectionNode>(hint, length);
  case extent_types_t::OBJECT_DATA_BLOCK:
    return alloc_ool_extent<ObjectDataBlock>(hint, length);
  case extent_types_t::RETIRED_PLACEHOLDER:
    ceph_assert(0 == "impossible");
    return CachedExtentRef();
  case extent_types_t::TEST_BLOCK:
    return alloc_ool_extent<TestBlock>(hint, length);

...
struct FLTreeOnode final : Onode, Value {
  ...
  struct Recorder : public ValueDeltaRecorder {
    ...
    void record_delta(NodeExtentMutable &value) {
      ...
      auto &encoded = get_encoded(value);
      ceph::encode(delta_op_t::UPDATE_ALL, encoded);
      encoded.append(bptr);
    }
  ...
  template <typename layout_func_t, typename recorder_func_t>
  void with_mutable_layout(
    Transaction &t,
    layout_func_t &&layout_func,
    recorder_func_t &&recorder_func) {
    assert(status != status_t::DELETED);
    auto p = prepare_mutate_payload<
      onode_layout_t,
      Recorder>(t);
    status = status_t::MUTATED;
    layout_func(*reinterpret_cast<onode_layout_t*>(p.first.get_write()));
    if (p.second) {
      recorder_func(p.first, *p.second);
    }
    status = status_t::STABLE;
  }

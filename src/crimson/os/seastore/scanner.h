...
public:
  ...
  void add_segment_manager(SegmentManager* sm) {
    auto& ref = segment_managers.emplace_back(sm);
    // segment managers must be added in the order of device id,
    // and device ids must be continuous
    assert(ref->get_device_id() == segment_managers.size() - 1);
  }

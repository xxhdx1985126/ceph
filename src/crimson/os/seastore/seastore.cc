...
seastar::future<> SeaStore::get_shard_nums(Device &device)
{
  ...
  if (done == -1) {
    ...
    if (store_shard_nums) {
      ceph_assert(store_shard_nums == shard_nums);
    }
...
seastar::future<uint32_t> SeaStore::start()
{
  ...
  if (cache_exists) {
    ...
    while (auto de = co_await lister()) {
      ...
      if (!primary_device ||
          primary_device->get_device_id() > p_cache_dev->get_device_id()) {
  ...
  if (!cache_exists && crimson::common::get_conf<bool>(
    ...
    std::string path = fmt::format("{}/block.0", cache_dev_path);
  ...
  while (auto de = co_await lister()) {
    ...
    if (cache_devices.empty() &&
        (!primary_device ||
         primary_device->get_device_id() > p_data_dev->get_device_id())) {

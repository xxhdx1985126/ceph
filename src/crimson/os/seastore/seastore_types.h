...
constexpr internal_segment_id_t SM_ID_MASK =
  0xF << (std::numeric_limits<internal_segment_id_t>::digits - device_id_len);
// default internal segment id
constexpr internal_segment_id_t DEFAULT_INTERNAL_SEG_ID =
...
struct segment_id_t {
  ...
  constexpr segment_id_t(internal_segment_id_t segment)
    : segment(segment) {}
  ...
  segment_id_t operator+(internal_segment_id_t t) {
    return {segment + t};
  }
...
struct __attribute((packed)) segment_id_le_t {
  ceph_le32 segment = ceph_le32(DEFAULT_INTERNAL_SEG_ID);
...
constexpr segment_id_t ZERO_SEG_ID =
  {std::numeric_limits<internal_segment_id_t>::max() - 5};
constexpr segment_id_t DELAYED_TEMP_SEG_ID =
  {std::numeric_limits<internal_segment_id_t>::max() - 6};

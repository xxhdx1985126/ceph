...
SegmentManager::read_ertr::future<uint32_t> BlockSegmentManager::get_shard_nums()
{
  return open_device(
    ...
    ceph_assert(sb.config.spec.dtype == dtype);
...
BlockSegmentManager::mount_ret BlockSegmentManager::shard_mount()
{
  ...
  return open_device(
    ...
    ceph_assert(device_id == sb.config.spec.id);
    ceph_assert(dtype == sb.config.spec.dtype);
  ...
  device_config_t sm_config)
{
  ...
  ceph_assert(device_id == superblock.config.spec.id);
  INFO("{} path={}, {}",

...
LBACursorImpl::base_iertr::future<> LBACursorImpl::refresh(
  cursor_stats_t &stats)
{
  LOG_PREFIX(LBACursorImpl::refresh);
  return with_btree<LBABtree>(
    ctx.cache,
    ctx,
    ...
    if (!parent->is_valid()) {
      ...
      TRACET("cursor {} parent is invalid, re-search from scratch",
	     c.trans, *(LBACursor*)this);
      return btree.lower_bound(c, this->get_laddr()
      ).si_then([this](LBABtree::iterator iter) {
	auto leaf = iter.get_leaf_node();
	parent = leaf;
	modifications = leaf->modifications;
	pos = iter.get_leaf_pos();
    ...
    bool viewable = !got_new;
    TRACET("cursor: {} viewable: {}",
      c.trans, *(LBACursor*)this, viewable);
    if (got_new) {
      leaf = l;
	...
	auto iter = LBALeafNode::iterator(leaf.get(), pos);
	ceph_assert(iter.get_key() == key);
	ceph_assert(iter.get_val() == val);
	assert(is_viewable());
      }

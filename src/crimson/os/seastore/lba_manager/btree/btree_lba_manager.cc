...
{
  assert(root_block->pending_lba_root_node
    ? root_block->is_mutation_pending() : true);
  auto lba_root = root_block->pending_lba_root_node
    ? root_block->pending_lba_root_node : root_block->lba_root_node;
  if (lba_root) {
...
template <bool pending_root_node, typename T, typename ROOT>
void set_phy_tree_root_node(RootBlockRef &root_block, ROOT* lba_root) {
  if constexpr (pending_root_node) {
    root_block->pending_lba_root_node = lba_root;
  } else {
    root_block->lba_root_node = lba_root;
  }

  if (likely(lba_root != nullptr)) {
...
BtreeLBAManager<leaf_has_children>::mkfs_ret
  ...
  Transaction &t)
{
  ...
  return cache.get_root(t).si_then([this, &t](auto croot) {
    ...
    croot->pending_lba_root_node = root_node.get();
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::get_mappings_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::get_mapping_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::alloc_extent_ret
  ...
  LogicalCachedExtent* nextent)
{
  ...
  return crimson::os::seastore::with_btree_state<LBABtree<leaf_has_children>, state_t>(
	...
	[this, &state, len, addr, &t, hint, FNAME,
	lookup_attempts, nextent](auto &pos) {
...
template <bool leaf_has_children>
typename BtreeLBAManager<leaf_has_children>::base_iertr::future<>
_init_cached_extent(
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::init_cached_extent_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::scan_mappings_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::rewrite_extent_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::update_mapping_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::get_physical_extent_if_live_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::update_refcount_ret
...
template <bool leaf_has_children>
BtreeLBAManager<leaf_has_children>::_update_mapping_ret

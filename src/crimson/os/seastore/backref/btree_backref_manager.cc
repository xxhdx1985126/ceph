...
const CachedExtentRef get_phy_tree_root_node<
  ...
  assert(root_block->pending_backref_root_node
    ? root_block->is_mutation_pending() : true);
  auto backref_root = root_block->pending_backref_root_node
    ? root_block->pending_backref_root_node : root_block->backref_root_node;
  if (backref_root) {
...
void set_phy_tree_root_node(RootBlockRef &root_block, ROOT* backref_root) {
  if constexpr (pending_root_node) {
    root_block->pending_backref_root_node = backref_root;
  } else {
    root_block->backref_root_node = backref_root;
  }
  if (likely(backref_root != nullptr)) {
  ...
  Transaction &t)
{
  ...
  return cache.get_root(t).si_then([this, &t](auto croot) {
    ...
    croot->pending_backref_root_node = root_node.get();

...
public:
  ...
  remap_pin_ret remap_pin(
	...
	fut = fut.si_then([this, &t, &pin] {
	  ...
	  if (ext) {
		...
		epm->mark_space_free(ext->get_paddr(), ext->get_length());
	  ...
	  for (auto &remap : remaps) {
	    ...
	    if (rbm_mutable_ext) {
	      epm->mark_space_used(remap_paddr, remap_len);
	    }
  ...
  remap_extent(
    ...
    return fut.si_then([this, &t, paddr, orig_gen, orig_hint,
			  remaps=std::move(remaps)](auto extent) mutable {

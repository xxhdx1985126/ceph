...
public:
  ...
  template <typename T>
  punch_mappings_ret punch_in_mapping_hole(
    Transaction &t,
    laddr_t laddr,
    objaddr_t len,
    LBAMapping mapping)
  {
    LOG_PREFIX(TransactionManager::punch_in_mapping_hole);
    ...
    bool left)
  {
    ...
    SUBDEBUGT(seastore_tm, "{} {} {}",
      t, pivot, mapping, left ? "LEFT" : "RIGHT");
    assert(mapping.is_indirect() || mapping.is_data_stable());
  ...
  /*
   * punch_in_hole_mappings
   *
   * remove the mappings that are completely inside the hole
   */
  punch_mappings_ret punch_in_hole_mappings(
    Transaction &t,
    laddr_t start,
    objaddr_t len,
    LBAMapping mapping)
  {
    LOG_PREFIX(TransactionManager::punch_in_hole_mappings);
	...
	return remove(t, std::move(mapping)
	  ...
	  crimson::ct_error::assert_all{
	    "punch_in_hole_mappings hit invalid error"
	  }
	);
      }).si_then([&mapping] {

...
std::string DaemonServer::_get_osd_device(int osd_id)
{
  ...
  if (!devices.empty()) {
    ...
    return boost::algorithm::join(devices, ",");
  }
  ...
  if (devices_meta != daemon->metadata.end() && !devices_meta->second.empty()) {
    ...
    for (auto& devname : devnames) {
      if (!daemon->hostname.empty()) {
        devname = daemon->hostname + ":" + devname;
      }
    }
    return boost::algorithm::join(devnames, ",");
  }

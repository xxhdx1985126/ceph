...
std::string fail_slow_cohort(const OSDMap& osdmap, int osd)
{
  const char *class_name = nullptr;
  if (osdmap.crush) {
    class_name = osdmap.crush->get_item_class(osd);
  }
  return std::string{"device_class:"} +
...
std::vector<fail_slow_device_score> find_fail_slow_devices(
  ...
  const std::function<std::string(int)>& get_osd_device)
{
  ...
  for (const auto& [osd, score] : scored_osds) {
    osds_by_device[get_osd_device(osd)].push_back(score);
  }
  ...
  for (auto& [device, osds] : osds_by_device) {
    ...
    if (*device_score < config.score_threshold ||

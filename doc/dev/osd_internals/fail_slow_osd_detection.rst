...
degradation caused by a very small number of disks with abnormally high I/O
latency. These devices may still respond within one or two seconds, which is
slow enough to hurt client workloads but not slow enough for the OSD to be
marked down.

This is especially damaging for VM workloads because VM data is distributed
across many OSDs. A single slow disk can therefore add latency to requests for
many unrelated VMs.
...
For bottom-up OSD or disk detection, use device-oriented latency such as
...
::
   ...
   disk_score = median(osd_score[osd] for osd on disk)

This helps identify the physical disk as the likely failing unit rather than
...
not on the disk is unnecessary for the primary detector. A leave-one-disk-out
...
* What exact score threshold and persistence window should raise a health
  warning?
* Should the first implementation only report suspects, or should it optionally
  mark them out behind an explicit configuration flag?
* What is the best cohort definition for pools that share OSDs or use device
  classes inconsistently?
* Which OSD states beyond recovery, backfill, deep scrub, and PG split/merge
  should exclude samples from the baseline?
* How should the detector behave when too few steady-state OSDs are available
  to calculate a reliable median and MAD?
